...
We will see a quick setup to configure the ``tool_vscode`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_vscode`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_vscode`` formulas under ``/srv/salt/tool_vscode/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_vscode/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_vscode/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_vscode/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_vscode/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_vscode`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_vscode`` formulas under ``/srv/salt/tool_vscode/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_vscode/parameters/roles
...
We create a configuration for the role ``tool_vscode/server`` in ``/srv/salt/tool_vscode/parameters/roles/tool_vscode/server.yaml``:
...
We create another configuration for the role ``tool_vscode/client`` in ``/srv/salt/tool_vscode/parameters/roles/tool_vscode/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_vscode/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_vscode/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_vscode/parameters/dns:domain/  '/srv/salt/tool_vscode/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-vscode-as-json:
      test.nop:
        - name: {{ vscode | json }}
...
.. code-block:: sls
    ...
    vscode-config-file-file-managed:
      file.managed:
        - name: {{ vscode.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='vscode-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``vscode`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ vscode['winner'] }}
